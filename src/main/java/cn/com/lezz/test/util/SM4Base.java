...
public class SM4Base {
    ...
    private final byte[] Sbox = { (byte) 0xd6, (byte) 0x90, (byte) 0xe9, (byte) 0xfe,
    ...
    private void SM4Crypt(byte[] Input, byte[] Output, int[] rk) {
        int r, mid;
        int[] x = new int[4];
        int[] tmp = new int[4];
        for (int i = 0; i < 4; i++) {
    ...
    public int sm4(byte[] in, byte[] key, byte[] out, int cryptFlag) {
        int inLen = in.length;
        if (inLen == 0 || (inLen % BLOCK) > 0) {
            return 0;
        }

        if (out == null || out.length < inLen) {
            out = new byte[inLen];
        }
        ...
        int index = 0;
        while (index < inLen) {
            input = Arrays.copyOfRange(in, index, index + BLOCK);
            SM4Crypt(input, output, round_key);
            System.arraycopy(output, 0, out, index, BLOCK);
